...
import MainInfo from "@ncl/app/components/dashboard/prepare-documents/children/main-info";
import GridRowInfo from "@ncl/app/components/dashboard/prepare-documents/children/grid-row-info";
...
export default function PrepareDocumentItem({ documentId }: Props) {
  const router = useRouter();
  const { user } = useUser();
  const { document, setDocument } = usePrepareDocument();
  ...
  useEffect(() => {
    ...
    if (user) {
      ...
      fetchRequest<PrepareDocument>(
        ...
        .catch((error) => {
          console.error(error);
          setSnackbarData({ open: true, message: error, messageType: "error" });
        })
    ...
    <div className="mt-2 print:mt-0">
      ...
      {document && (
        <div className="document">
          ...
          />
          <GridRowInfo
            groupName="Suspensión y dirección"
            items={[
              {
          ...
          <Button className="print:!hidden my-4" onClick={() => window.print()}>
            Guardar o imprimir documento
          </Button>
        </div>
      )}
    </div>
